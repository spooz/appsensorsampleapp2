...
public class SpringSecurityUserManager implements UserManager {
    ...
    public void disable(User appSensorUser) {
        ...
        if (userDetailsManager == null) {
            logger.warn("Could not disable user [" + appSensorUser.getUsername() + "] since the spring security " +
        ...
        if (springUser == null) {
            logger.warn("Could not disable user [" + appSensorUser.getUsername() + "] because the user could not " +
                    "be found by lookup" );
            return;
        }
        ...
        logger.info("After disabling user <{}>, also logging out so the disable gets triggered.", springUser.getUsername());
        userResponseCache.setUserLoggedOut(appSensorUser.getUsername());
    }
